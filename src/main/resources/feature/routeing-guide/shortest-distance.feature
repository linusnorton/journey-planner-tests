...
Feature: A journey within 3 miles of the shortest distance is valid
  ...
  # JP-489 Aldgate East (Underground) to Hastings via Brough
  Scenario: Total distance is calculated correctly for journeys starting with non-rail (including timetabled-bus) legs
    Given a journey:
      | ZAE | depart | tube  |
      | KGX | change | train |
      | BUH | change | train |
